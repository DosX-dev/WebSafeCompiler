...
function obfuscateHtml(html) {
    try {
        // Correctly skip <script> and <style> tags, do not touch their content
        const tagRegex = /(<script[\s\S]*?<\/script>|<style[\s\S]*?<\/style>|<[^>]+>)/gi;
        let lastIndex = 0, result = '', match;
        while ((match = tagRegex.exec(html)) !== null) {
            // Add text between tags as is
            if (match.index > lastIndex) result += html.slice(lastIndex, match.index);
            const tag = match[0];
            if (/^<script[\s\S]*?<\/script>$/i.test(tag) || /^<style[\s\S]*?<\/style>$/i.test(tag)) {
                // Return <script> and <style> blocks as is
                result += tag;
            } else if (/^<\w+/.test(tag)) {
                // Process normal opening/self-closing tags
                const tagMatch = tag.match(/^<(\w+)(\s[^>]*?)?(\s*\/?)>/);
                if (tagMatch) {
                    const tagName = tagMatch[1], attributes = tagMatch[2], closingSlash = tagMatch[3];
                    if (!attributes || ['meta', 'noscript'].includes(tagName)) {
                        result += `<${tagName}${attributes || ''}${closingSlash || ''}>`;
                    } else {
                        const attrArray = attributes.trim().match(/\S+="[^"]*"|\S+='[^']*'|\S+/g) || [];

                        const obfuscatedAttrs = attrArray.map(attr => {
                            let obfuscatedAttr = attr;
                            // Obfuscate class attribute by adding random class names
                            if (/^class=['"]/.test(attr)) {
                                const
                            ...
                            // Add random strings before and after each attribute
                        ...
                        result += `<${tagName} ${obfuscatedAttrs}${closingSlash || ''}>`;
                    }
                } else {
                    // Fallback: if not parsed, return as is
                    result += tag;
                }
            } else {
                // Any other tag, return as is
                result += tag;
            }
            lastIndex = tagRegex.lastIndex;
        }
        // Add the rest of the text after the last tag
        if (lastIndex < html.length) result += html.slice(lastIndex);
        return result;
    } catch (error) {
